"""
Model Training Module for Distributed Formation Control (Version 2.1)
...
- K-fold cross-validation with varying parameters
- Comprehensive testing of temporal window configurations
...
def evaluate_model(model: nn.Module, test_loader: DataLoader, criterion: nn.Module, device: torch.device) -> Tuple[float, Dict[str, float]]:
    ...
    with torch.no_grad():
        for (static_features, shape_error, presence_error, leader_off), targets in test_loader:
            # Move data to device
            static_features = static_features.to(device)
            shape_error = shape_error.to(device)
            presence_error = presence_error.to(device)
...
def run_k_fold_tests_with_varying_params(
    model: nn.Module,
    static_features: np.ndarray,
    shape_error: np.ndarray,
    presence_error: np.ndarray,
    ...
    weight_decay: float,
    device: torch.device,
    include_size_formation: bool
) -> pd.DataFrame:
    """
    Run k-fold tests with varying temporal window parameters.
    ...
    Returns:
        pd.DataFrame: Results table with metrics for each configuration
    """
    modes = ['short', 'linear', 'long']
    results = []
    
    # Calculate total number of combinations
    total_combinations = len(modes) * max_windows * k
    current_combination = 0
    
    for mode in modes:
        for num_windows in range(1, max_windows + 1):
            print("\n--------------------------------------------------------------------")
            print(f"---------------------{mode} mode and {num_windows} window(s)---------------------")
            print("--------------------------------------------------------------------")
            ...
            results.append({
                'Mode': mode,
                'Windows': num_windows,
                'Test Loss (mean)': np.mean(test_losses_array),
                'Test Loss (std)': np.std(test_losses_array),
                'R2 Score (mean)': np.mean(mean_r2_per_fold),
                'R2 Score (std)': np.std(mean_r2_per_fold)
            })
            
            # Update and print progress
            current_combination += k
            print(f"\nProgress: {current_combination}/{total_combinations} combinations completed")
    
    # Convert results to DataFrame
    results_df = pd.DataFrame(results)
    
    # Save results to CSV
    results_df.to_csv('k_fold_tests.csv', index=False)
    
    return results_df

def get_split_type() -> Tuple[str, Optional[int], Optional[int], Optional[str]]:
    """
    Ask the user to choose between split types and get additional parameters if needed.
    
    Returns:
        Tuple[str, Optional[int], Optional[int], Optional[str]]: 
            Split type, K value (if k_fold), max windows (if k_fold_tests), and mode (if k_fold_tests)
    """
    ...
    print("3. One K-fold with traditional split")
    print("4. Set of K-fold with traditional split varying (a) Max windows quantity and (b) Temporal summary mode")
    ...
    if choice == "1":
        return "a_priori", None, None, None
    elif choice == "2":
        return "traditional", None, None, None
    elif choice == "3":
        while True:
            try:
        ...
        while True:
            try:
                max_windows = int(input("Enter the maximum number of windows (1-100): "))
                if 1 <= max_windows <= 100:
                    break
                else:
                    print("Invalid input. Please enter a number between 1 and 100.")
            except ValueError:
        ...
        return "k_fold_tests", k, max_windows, None
...
def main():
    ...
    # Step 1: Get split type (Question A)
    print("\nChoose the training type:")
    print("1. A priori split (using dataset_type field)")
    print("2. Traditional split (80% train, 10% validation, 10% test)")
    print("3. One K-fold with traditional split")
    ...
    # Step 2: Get temporal summary parameters for options 1, 2, or 3
    if choice in ["1", "2", "3"]:
        # Question B: Number of windows
        print("\nDefine the number of windows for the TemporalSummary layer:")
        print("This determines how many summary features will be extracted from each time series.")
        
        while True:
            try:
                num_outputs = int(input("Enter number of windows (1-100): "))
                if 1 <= num_outputs <= 100:
                    break
                else:
                    print("Invalid input. Please enter a number between 1 and 100.")
        ...
        mode = mode_map[mode_choice]
    # Removed the elif block for option 4 since num_outputs and mode will be varied during testing
    
    # Step 3: Get min_ratio and size_formation choice for all options
    if choice in ["1", "2", "3", "4"]:
        # Question D: Minimum ratio
        print("\nDefine the minimum ratio for window sizes:")
        print("This determines the smallest window size relative to the total time steps.")
        ...
        # Question E: Size formation feature
        print("\nDo you want to include the size_formation feature in the model?")
        print("1. Yes")
        print("2. No")
        
        while True:
            size_choice = input("Enter option number (1-2): ")
            if size_choice in ["1", "2"]:
                break
            else:
                print("Invalid input. Please enter 1 or 2.")
        
        include_size_formation = size_choice == "1"
    
    # Step 4: Get number of folds for options 3 or 4
    if choice in ["3", "4"]:
        # Question F: Number of folds
        while True:
            try:
                k = int(input("\nEnter the number of folds (3-10): "))
                if 3 <= k <= 10:
                    break
                else:
                    print("Invalid input. Please enter a number between 3 and 10.")
    ...
    # Step 5: Get max windows for option 4
    if choice == "4":
        # Question G: Maximum number of windows
        while True:
            try:
                max_windows = int(input("\nEnter the maximum number of windows (1-100): "))
                if 1 <= max_windows <= 100:
                    break
                else:
                    print("Invalid input. Please enter a number between 1 and 100.")
    ...
    if choice == "4":
        # Load all data for k-fold tests
        if include_size_formation:
            static_features = np.column_stack((data['size_formation'], data['comm_delay']))
        else:
            static_features = data['comm_delay'].reshape(-1, 1)
        ...
        # Run k-fold tests with varying parameters
        results_df = run_k_fold_tests_with_varying_params(
            FormationModel,
            static_features,
            shape_error,
            presence_error,
        ...
        print(f"\nResults saved to k_fold_tests.csv")
        ...
        (X_train_static, X_train_shape, X_train_presence, X_train_leader, y_train,
         X_val_static, X_val_shape, X_val_presence, X_val_leader, y_val,
         X_test_static, X_test_shape, X_test_presence, X_test_leader, y_test) = load_data(DATASET_PATH, choice, include_size_formation)
